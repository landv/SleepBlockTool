...
- 选中进程后一键结束：优先使用 `wmic`，若系统未启用/未安装 wmic 则自动回退到 `taskkill`，无需手动打开命令行。
...
- **结束进程方式自适应**：运行时通过 `where wmic` 检测 wmic 是否可用（结果缓存、只检测一次）；可用则用 `wmic`，不可用则自动改用系统自带的 `taskkill /IM xxx.exe /F`，日志中会标明实际使用的方式。
